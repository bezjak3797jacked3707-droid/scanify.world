  ...
  return (
    <div className="flex flex-col items-center justify-center py-24 gap-5">
      ...
      <div className="text-center">
        <p className="text-sm font-medium mb-1" style={{ color: "#555" }}>No scans this month</p>
        <p className="text-xs" style={{ color: "#333" }}>Your scanned items will appear here</p>
      </div>
...
export default function HistoryPage() {
  ...
  const sorted = useMemo<Scan[]>(() => {
    if (sort === "expensive") {
      return [...scans].sort((a, b) => parsePrice(b.current_value) - parsePrice(a.current_value));
    }
    return scans;
  }, [scans, sort]);
    ...
    return (
      <main className="min-h-screen flex items-center justify-center" style={{ background: "var(--color-black)" }}>
        <div className="text-center space-y-4">
          ...
          <p className="text-sm" style={{ color: "#555" }}>Loading history…</p>
        </div>
      </main>
    );
  }
          ...
          <div className="flex flex-col gap-3">
            {sorted.map((scan) => (
              <button
                key={scan.id}
                onClick={() => router.push(
                  scan.scan_type === "resell"
                    ? `/resell/result?scanId=${scan.id}`
                ...
                style={{
                  background: scan.scan_type === "resell"
                    ? "linear-gradient(135deg, rgba(201,168,76,0.06) 0%, var(--color-surface) 70%)"
                    : "var(--color-surface)",
                  border: scan.scan_type === "resell"
                    ? "1px solid rgba(201,168,76,0.2)"
                    : "1px solid var(--color-border)",
                }}
              >
                <div className="w-[72px] h-[72px] rounded-xl overflow-hidden flex-shrink-0" style={{ background: "#1a1a1a", border: "1px solid #1a1a1a" }}>
                  {scan.image_url ? (
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-start justify-between gap-2 mb-1.5">
                    <p className="font-semibold text-sm leading-tight truncate" style={{ color: "#ddd" }}>
                      {scan.name}
                    </p>
                    <span className="text-[10px] flex-shrink-0 px-2 py-0.5 rounded-full" style={{ color: "#555", background: "#1a1a1a", border: "1px solid #222" }}>
